...
- Capability behavior and field-level detail → [docs/resources.md](resources.md)
- Design rationale → [docs/adr/README.md](adr/README.md)
- Raynet API runtime semantics (auth, endpoints, filters) → the official [Raynet CRM v2 API documentation](https://app.raynet.cz/api/doc/index-en.html)
...
Every resource lives under `nodes/Raynet/{resource}/`. Most resources follow a 4-file pattern:
...
**Exception — activity resources:** `task/`, `call/`, `meeting/`, `email/`, `event/`, `letter/` each expose only `index.ts`. Their UI definitions and body building are generated by the shared factory in `activity/activityShared.ts` rather than per-resource files.
